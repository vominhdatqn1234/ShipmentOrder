...
export default function EditOrderDetail({
  ...
  const { orders, setOrders, updateOrderId } = useOrderSlice();
  ...
  // );
  const uuId = uuidv4();
  console.log("orderParent", orderParent, defaultValues);
    ...
    <>
      ...
      <FormAntDeisgn
        ...
        onFinish={handleSubmit(async (data: OrdersModel) => {
          ...
          const total = +data?.quantity * parseFloat(data?.price);
          ...
          const updatedOrdersArray = produce(
            ...
            (order: any) => {
              const indexToUpdate = order?.findIndex(
                (item: OrdersModel) => item.id === payload.id
              );
              if (indexToUpdate !== -1) {
          ...
          updateOrderId(orderParent?.id, updatedOrdersArray);

          const docRef = doc(contractRef, orderParent?.id);
