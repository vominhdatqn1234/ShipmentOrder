...
export default function WeddingAblumEditForm({
  type = "album",
  data,
  file,
  handleCancel,
  ...
  const albumPhotoItem =
    find(data?.[typeWedding[type]]?.gallery, { id: file?.id }) || {};
  console.log("albumPhotoItem", albumPhotoItem);
  const defaultUrlItem = {
    id: uuidv4(),
    uid: uuidv4(),
    status: "done",
    url: albumPhotoItem?.url,
    name: albumPhotoItem?.name,
    alt: albumPhotoItem?.alt,
  };
  ...
  const servicePageRef = collection(firestore, "servicePage");
  const mutation = useFirestoreCollectionMutation(servicePageRef);
  const queryClient = useQueryClient();

  const defaultValues = {
    title: albumPhotoItem?.title || "",
    description: albumPhotoItem?.description || "",
    gallery: albumPhotoItem?.gallery || [],
    url: [defaultUrlItem],
  };
  ...
  useEffect(() => {
    ...
    setFileList(albumPhotoItem?.gallery || []);
    ...
    <>
      ...
      <FormAntDeisgn
        ...
        onFinish={handleSubmit(async (values: any) => {
          const newAlbumPhotoItem = map(data?.albumPhoto?.gallery, (item) => {
            if (item.id === file.id) {
              return assign({}, item, {
          ...
          const payload = {
            ...data,
            [typeWedding[type]]: {
              ...data?.[typeWedding[type]],
              gallery: newAlbumPhotoItem,
            },
          };
          // const docRef = doc(servicePageRef, data.id);
      ...
      >
        ...
        <FormItem
          ...
          name="gallery"
          // valuePropName='fileList'
          label="Upload hình banner header"
        >
          <Dragger {...props}>
