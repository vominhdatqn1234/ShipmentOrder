...
const ProductsType = ({ orderDetail }: any) => {
  const [searchText, setSearchText] = useState("");
  const [searchedColumn, setSearchedColumn] = useState("");
  const searchInput = useRef<InputRef>(null);
  const { employees: employeeData, updateEmployeeId } = useEmployeeSlice();
  const isExistEmployee = find(employeeData, { id: orderDetail });
  const productTypeData = isExistEmployee?.productTypes || [];
  ...
  const handleDelete = (record: any) => async () => {
    const updatedProductTypesArray = produce(
      isExistEmployee?.productTypes,
      (productType: any) => {
        const indexToUpdate = productType?.findIndex(
          (item: ProductType) => item.id === record.id
        );
        if (indexToUpdate !== -1) {
          productType.splice(indexToUpdate, 1);
        }
      }
    );
    const docRef = doc(collectionRef, isExistEmployee?.id);
    await updateDoc(docRef, {
      ...isExistEmployee,
      productTypes: updatedProductTypesArray,
    });
    updateEmployeeId(isExistEmployee!.id, updatedProductTypesArray);
  };
  ...
  return (
    <div className="m-2 p-2 md:p-10 bg-white rounded-3xl">
      ...
      >
        <EditContractType
          parentEmployee={isExistEmployee}
